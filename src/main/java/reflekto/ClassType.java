...
public class ClassType extends AbstractType implements ObjectType, GenericDeclaration {
	...
	public ClassType(Class<?> clazz, FullReflector creator) {
		super(clazz, creator);
	...
	public ClassType(ClassType original, List<Type> actualTypeParameters, boolean isErasure) {
	...
	public TypeName getGenericName() {
		return new AbstractTypeName() {
			...
			public String buildName(String className, TypeName.Kind kind) {
				if (actualTypeParameters.get().isEmpty() || ClassType.this.isErasure()) {
					return className;
				}
				StringBuilder s = new StringBuilder();
				s.append(className);
